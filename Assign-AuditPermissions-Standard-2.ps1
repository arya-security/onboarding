...
param(
    ...
    [Parameter(Mandatory = $false, HelpMessage = "IAM audit user name. Defaults to 'ARYA_Security_Audit'.")]
...
# ── Constants ────────────────────────────────────────────────────────────────
$script:ROLE_NAME = "ARYASecurityAuditRole"
$script:CUSTOM_POLICY_NAME = "SecurityAuditCustomEdge"
...
$script:RoleReused = $false
$script:RoleUpdated = $false
...
function Resolve-DefaultUserName {
    $preferred = "ARYA Security Audit"
    if ($preferred -match '^[a-zA-Z0-9_+=,.@-]{1,64}$') {
        return $preferred
    }
    return "ARYA_Security_Audit"
}
...
function Ensure-RolePolicyAttached {
    param(
    ...
    $attachedPolicies = Invoke-AwsCli iam list-attached-role-policies --role-name $RoleName --output json
    if ($attachedPolicies.ExitCode -eq 0) {
        $alreadyAttached = @((ConvertFrom-AwsJsonOrNull $attachedPolicies.Raw).AttachedPolicies | Where-Object { $_.PolicyArn -eq $PolicyArn }) | Select-Object -First 1
        if ($alreadyAttached) {
            Write-Host "  ✓ $FriendlyName already attached" -ForegroundColor Green
            return $true
        }
    }

    $attachResult = Invoke-AwsCli iam attach-role-policy --role-name $RoleName --policy-arn $PolicyArn --output json
    if ($attachResult.ExitCode -ne 0) {
        return $false
    }

    Write-Host "  ✓ $FriendlyName attached" -ForegroundColor Green
    return $true
}
...
function Confirm-RoleConflictOrResolve {
    while ($true) {
        ...
        $script:RoleArn = $existingRole.Role.Arn
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "Choose how to continue:" -ForegroundColor Yellow
        Write-Host "  [U] Use existing role as-is"
        Write-Host "  [P] Patch / update existing role trust policy and session duration"
        Write-Host "  [R] Recreate this IAM role (delete existing role and create a new one with the same name)"
        Write-Host "  [A] Abort"

        $choice = (Read-Host "Enter U, P, R, or A").Trim().ToUpperInvariant()

        switch ($choice) {
            "U" {
                $script:RoleReused = $true
                $script:RoleUpdated = $false
                Write-Host "  ✓ Will continue with the existing IAM role as-is" -ForegroundColor Green
                break
            }
            "P" {
                $script:RoleReused = $true
                $script:RoleUpdated = $true
                Write-Host "  ✓ Will update the existing IAM role and continue" -ForegroundColor Green
                break
            }
            "R" {
            ...
            default {
                Write-Host "  Invalid choice. Please enter U, P, R, or A." -ForegroundColor Yellow
                continue
            }
        }
    }
...
function Show-OnboardingSummary {
    ...
    Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    $roleStatus = if ($script:RoleCreated) { "created" } elseif ($script:RoleUpdated) { "existing role updated" } elseif ($script:RoleReused) { "existing role reused" } else { "not created" }
    Write-Host ("  IAM Role                 : {0}" -f $roleStatus)
...
# ── 5. Create or update IAM Role with trust policy ────────────────────────────
Write-Step 5 "Creating or updating IAM Role '$($script:ROLE_NAME)' with trust policy"
...
try {
    ...
    if ($script:RoleReused -and -not $script:RoleUpdated) {
        Write-Host "  ✓ Reusing existing IAM role without changes: $($script:RoleArn)" -ForegroundColor Green
    }
    elseif ($script:RoleUpdated) {
        if ($PSCmdlet.ShouldProcess("IAM Role '$($script:ROLE_NAME)'", "Update trust policy and session duration")) {
            $updateTrustResult = Invoke-AwsCli iam update-assume-role-policy `
                --role-name $script:ROLE_NAME `
                --policy-document "file://$trustFile"

            if ($updateTrustResult.ExitCode -ne 0) {
                Exit-WithError "Failed to update role trust policy: $($updateTrustResult.Raw)"
            }

            $updateRoleResult = Invoke-AwsCli iam update-role `
                --role-name $script:ROLE_NAME `
                --max-session-duration $SessionDuration `
                --description "Read-only role for ARYA security audit."

            if ($updateRoleResult.ExitCode -ne 0) {
                Add-WarningItem "Failed to update role session duration/description. Continuing with the existing role settings. Reason: $($updateRoleResult.Raw)"
            }

            $roleReadBack = Invoke-AwsCli iam get-role --role-name $script:ROLE_NAME --output json
            if ($roleReadBack.ExitCode -ne 0) {
                Exit-WithError "Role update succeeded but the role could not be read back afterwards."
            }

            $script:RoleArn = (ConvertFrom-AwsJsonOrNull $roleReadBack.Raw).Role.Arn
            if (-not $script:RoleArn) {
                Exit-WithError "Role update succeeded but its ARN could not be parsed."
            }

            Write-Host "  ✓ Existing role updated: $($script:RoleArn)" -ForegroundColor Green
        }
    }
    else {
...
if ($PSCmdlet.ShouldProcess("SecurityAudit → $($script:ROLE_NAME)", "Ensure attached")) {
    $securityAuditOk = Ensure-RolePolicyAttached -RoleName $script:ROLE_NAME -PolicyArn $script:SECURITY_AUDIT_ARN -FriendlyName "SecurityAudit"
    if (-not $securityAuditOk) {
        if ($script:RoleCreated) {
            Write-Host "  ✗ Attach failed — rolling back newly created role" -ForegroundColor Red
            [void](Invoke-AwsCli iam delete-role --role-name $script:ROLE_NAME)
        }
        Exit-WithError "Failed to attach SecurityAudit to role '$($script:ROLE_NAME)'."
    }
...
try {
    ...
    if ($PSCmdlet.ShouldProcess("Custom policy '$($script:CUSTOM_POLICY_NAME)'", "Create")) {
        ...
        if ($policyResult.ExitCode -ne 0) {
            $localPolicies = Invoke-AwsCli iam list-policies --scope Local --output json
            if ($localPolicies.ExitCode -eq 0) {
                $existingCustom = @((ConvertFrom-AwsJsonOrNull $localPolicies.Raw).Policies | Where-Object { $_.PolicyName -eq $script:CUSTOM_POLICY_NAME }) | Select-Object -First 1
                if ($existingCustom -and $existingCustom.Arn) {
                    $script:CustomPolicyArn = $existingCustom.Arn
                    Write-Host "  ✓ Reusing existing custom policy: $($script:CustomPolicyArn)" -ForegroundColor Green
                }
            }

            if (-not $script:CustomPolicyArn) {
        ...
        if ($script:CustomPolicyArn) {
            $customAttachOk = Ensure-RolePolicyAttached -RoleName $script:ROLE_NAME -PolicyArn $script:CustomPolicyArn -FriendlyName "Custom policy"
            if (-not $customAttachOk) {
                Add-WarningItem "Failed to attach custom policy. Continuing with SecurityAudit only."
            }
        }
    }
}
