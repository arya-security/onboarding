...
.DESCRIPTION
    Cloud Shell oriented removal script aligned to the assign flow:
      ...
      - The script discovers the relevant cleanup scopes automatically from the existing app/service principal
...
function Discover-CleanupScopes {
    param([string]$PrincipalObjectId)
    ...
    $readerScopes = @(
        $allAssignments |
        Where-Object { $_.roleDefinitionName -eq "Reader" -and $_.scope } |
        Select-Object -ExpandProperty scope -Unique
    ) | ForEach-Object { Normalize-ScopeForLookup -ScopeValue $_ }

    $kvScopes = @(
        $allAssignments |
        Where-Object { $_.roleDefinitionName -eq "Key Vault Reader" -and $_.scope } |
        Select-Object -ExpandProperty scope -Unique
    ) | ForEach-Object { Normalize-ScopeForLookup -ScopeValue $_ }

    $edgeScopes = @(
        $allAssignments |
        Where-Object { $_.roleDefinitionName -eq $EDGE_ACTIONS_ROLE_NAME -and $_.scope } |
        Select-Object -ExpandProperty scope -Unique
    ) | ForEach-Object { Normalize-ScopeForLookup -ScopeValue $_ }
    ...
    [pscustomobject]@{
        ReaderScopes    = @($readerScopes | Sort-Object -Unique)
        KeyVaultScopes  = @($kvScopes | Sort-Object -Unique)
        EdgeScopes      = @($edgeScopes | Sort-Object -Unique)
...
$cleanupScopes = $null
if ($spObjectId) {
    Write-Step 2 "Discovering cleanup scopes from existing assignments"
    $cleanupScopes = Discover-CleanupScopes -PrincipalObjectId $spObjectId

    if ($cleanupScopes.NeedsRootAccess) {
        [void](Try-ElevateAccess)
    }

    Write-Host ("  Reader scopes     : {0}" -f $(if (@($cleanupScopes.ReaderScopes).Count -gt 0) { ($cleanupScopes.ReaderScopes -join ', ') } else { "none" })) -ForegroundColor DarkGray
    Write-Host ("  Key Vault scopes  : {0}" -f $(if (@($cleanupScopes.KeyVaultScopes).Count -gt 0) { ($cleanupScopes.KeyVaultScopes -join ', ') } else { "none" })) -ForegroundColor DarkGray
    Write-Host ("  Edge role scopes  : {0}" -f $(if (@($cleanupScopes.EdgeScopes).Count -gt 0) { ($cleanupScopes.EdgeScopes -join ', ') } else { "none" })) -ForegroundColor DarkGray
} else {
    $cleanupScopes = [pscustomobject]@{
        ReaderScopes = @()
        KeyVaultScopes = @()
        EdgeScopes = @()
...
if ($spObjectId) {
    Remove-RbacAssignmentsByRoleName -PrincipalObjectId $spObjectId -RoleName "Reader" -Scopes @($cleanupScopes.ReaderScopes)
} else {
...
if ($spObjectId) {
    Remove-RbacAssignmentsByRoleName -PrincipalObjectId $spObjectId -RoleName "Key Vault Reader" -Scopes @($cleanupScopes.KeyVaultScopes)
} else {
...
if ($spObjectId) {
    Remove-RbacAssignmentsByRoleName -PrincipalObjectId $spObjectId -RoleName $EDGE_ACTIONS_ROLE_NAME -Scopes @($cleanupScopes.EdgeScopes)
} else {
...
Remove-SafeEdgeRoleDefinitions -ExpectedAssignableScopes @($cleanupScopes.EdgeScopes)
...
Write-Host ("  Reader cleanup scopes   : {0}" -f $(if (@($cleanupScopes.ReaderScopes).Count -gt 0) { ($cleanupScopes.ReaderScopes -join ', ') } else { "none" }))
Write-Host ("  Key Vault cleanup scopes: {0}" -f $(if (@($cleanupScopes.KeyVaultScopes).Count -gt 0) { ($cleanupScopes.KeyVaultScopes -join ', ') } else { "none" }))
Write-Host ("  Edge role scopes        : {0}" -f $(if (@($cleanupScopes.EdgeScopes).Count -gt 0) { ($cleanupScopes.EdgeScopes -join ', ') } else { "none" }))
