...
$attachedRolePoliciesResult = Invoke-AwsCli iam list-attached-role-policies --role-name $script:ROLE_NAME --output json
if ($attachedRolePoliciesResult.ExitCode -eq 0) {
    $attachedRolePolicies = @((ConvertFrom-AwsJsonOrNull $attachedRolePoliciesResult.Raw).AttachedPolicies)
}
else {
    Add-WarningItem "Could not list currently attached role policies before attach/update. Continuing anyway. Reason: $($attachedRolePoliciesResult.Raw)"
}
