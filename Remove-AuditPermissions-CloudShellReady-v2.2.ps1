...
.DESCRIPTION
    Cloud Shell oriented removal script aligned to the assign flow:
      - TenantId and AppDisplayName are mandatory
      - No Scope / Subscriptions inputs are required
      - The script discovers the relevant cleanup scopes and exact role assignment IDs automatically
      - If root-level cleanup is needed, it tries elevateAccess automatically
      ...
      - Custom role deletion is safer: delete only when the exact role definition matches
        the expected actions and the discovered assignable scopes, and only after role assignments are removed
#>
...
function Try-ElevateAccess {
    Write-Host "Trying to elevate access for possible root-scope cleanup..." -ForegroundColor Yellow
...
function Get-AllRoleAssignmentsForPrincipal {
    ...
    if ($res.ExitCode -ne 0) {
        Add-WarningItem "Failed to list RBAC role assignments for the service principal."
        return @()
    }
...
function Normalize-ScopeForLookup {
...
function Discover-CleanupData {
    param([string]$PrincipalObjectId)

    $allAssignments = @(Get-AllRoleAssignmentsForPrincipal -PrincipalObjectId $PrincipalObjectId)

    $readerAssignments = @(
        $allAssignments |
        Where-Object { $_.roleDefinitionName -eq "Reader" -and $_.id }
    )

    $kvAssignments = @(
        $allAssignments |
        Where-Object { $_.roleDefinitionName -eq "Key Vault Reader" -and $_.id }
    )

    $edgeAssignments = @(
        $allAssignments |
        Where-Object { $_.roleDefinitionName -eq $EDGE_ACTIONS_ROLE_NAME -and $_.id }
    )

    $readerScopes = @($readerAssignments | ForEach-Object { Normalize-ScopeForLookup -ScopeValue $_.scope } | Sort-Object -Unique)
    $kvScopes     = @($kvAssignments | ForEach-Object { Normalize-ScopeForLookup -ScopeValue $_.scope } | Sort-Object -Unique)
    $edgeScopes   = @($edgeAssignments | ForEach-Object { Normalize-ScopeForLookup -ScopeValue $_.scope } | Sort-Object -Unique)

    $allScopes = @($readerScopes + $kvScopes + $edgeScopes | Sort-Object -Unique)
    $containsRootMg = @($allScopes | Where-Object { $_ -eq "/providers/Microsoft.Management/managementGroups/$TenantId" }).Count -gt 0

    [pscustomobject]@{
        ReaderAssignments = @($readerAssignments)
        KeyVaultAssignments = @($kvAssignments)
        EdgeAssignments = @($edgeAssignments)
        ReaderScopes    = @($readerScopes)
        KeyVaultScopes  = @($kvScopes)
        EdgeScopes      = @($edgeScopes)
        NeedsRootAccess = $containsRootMg
    }
}

function Remove-RoleAssignmentsByDiscoveredIds {
    param(
        [string]$RoleName,
        [object[]]$Assignments
    )
    ...
    $items = @($Assignments)
    if ($items.Count -eq 0) {
        Write-Host "  → $RoleName assignments not found (skipped)" -ForegroundColor Yellow
        return
    }

    foreach ($assignment in $items) {
        $assignmentId = $assignment.id
        $scopeItem = Normalize-ScopeForLookup -ScopeValue $assignment.scope
        if (-not $assignmentId) { continue }

        if ($PSCmdlet.ShouldProcess("$RoleName at $scopeItem", "Delete discovered role assignment")) {
...
function Get-SafeEdgeRoleDefinitions {
    ...
    $expectedScopes = @($ExpectedAssignableScopes | Sort-Object -Unique)
    $safe = @()
    ...
    foreach ($def in @($defs)) {
        ...
        $actionsDiff = @(Compare-Object -ReferenceObject (@($ExpectedEdgeActions | Sort-Object)) -DifferenceObject $permActions)
        $scopesDiff  = @(Compare-Object -ReferenceObject $expectedScopes -DifferenceObject $scopes)

        $actionsMatch = (@($permActions).Count -eq @($ExpectedEdgeActions).Count) -and ($actionsDiff.Count -eq 0)
        $scopesMatch  = ($scopesDiff.Count -eq 0)
...
$cleanup = $null
if ($spObjectId) {
    Write-Step 2 "Discovering cleanup scopes and assignment IDs from existing assignments"
    $cleanup = Discover-CleanupData -PrincipalObjectId $spObjectId

    if ($cleanup.NeedsRootAccess) {
        [void](Try-ElevateAccess)
    }

    Write-Host ("  Reader scopes     : {0}" -f $(if (@($cleanup.ReaderScopes).Count -gt 0) { ($cleanup.ReaderScopes -join ', ') } else { "none" })) -ForegroundColor DarkGray
    Write-Host ("  Key Vault scopes  : {0}" -f $(if (@($cleanup.KeyVaultScopes).Count -gt 0) { ($cleanup.KeyVaultScopes -join ', ') } else { "none" })) -ForegroundColor DarkGray
    Write-Host ("  Edge role scopes  : {0}" -f $(if (@($cleanup.EdgeScopes).Count -gt 0) { ($cleanup.EdgeScopes -join ', ') } else { "none" })) -ForegroundColor DarkGray
} else {
    $cleanup = [pscustomobject]@{
        ...
        KeyVaultScopes = @()
        EdgeScopes = @()
        NeedsRootAccess = $false
    }
}

Write-Step 3 "Removing Global Reader assignment"
if ($spObjectId) {
...
Write-Step 4 "Removing Reader role assignments"
if ($spObjectId) {
    Remove-RoleAssignmentsByDiscoveredIds -RoleName "Reader" -Assignments @($cleanup.ReaderAssignments)
} else {
    Add-WarningItem "Skipped Reader cleanup because Service Principal was not found."
}

Write-Step 5 "Removing Key Vault Reader role assignments"
if ($spObjectId) {
    Remove-RoleAssignmentsByDiscoveredIds -RoleName "Key Vault Reader" -Assignments @($cleanup.KeyVaultAssignments)
} else {
    Add-WarningItem "Skipped Key Vault Reader cleanup because Service Principal was not found."
}

Write-Step 6 "Removing Audit Edge Actions role assignments"
if ($spObjectId) {
    Remove-RoleAssignmentsByDiscoveredIds -RoleName $EDGE_ACTIONS_ROLE_NAME -Assignments @($cleanup.EdgeAssignments)
} else {
    Add-WarningItem "Skipped Audit Edge Actions role-assignment cleanup because Service Principal was not found."
}

Write-Step 7 "Removing Audit Edge Actions role definition safely"
Remove-SafeEdgeRoleDefinitions -ExpectedAssignableScopes @($cleanup.EdgeScopes)

Write-Step 8 "Deleting App Registration"
...
Write-Host ("  Reader cleanup scopes   : {0}" -f $(if (@($cleanup.ReaderScopes).Count -gt 0) { ($cleanup.ReaderScopes -join ', ') } else { "none" }))
Write-Host ("  Key Vault cleanup scopes: {0}" -f $(if (@($cleanup.KeyVaultScopes).Count -gt 0) { ($cleanup.KeyVaultScopes -join ', ') } else { "none" }))
Write-Host ("  Edge role scopes        : {0}" -f $(if (@($cleanup.EdgeScopes).Count -gt 0) { ($cleanup.EdgeScopes -join ', ') } else { "none" }))
