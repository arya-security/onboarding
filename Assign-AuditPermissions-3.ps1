...
.PARAMETER UserName
    ...
    If omitted, defaults to "ARYASecurityAudit".
...
function Get-PerUserRoleName {
    param([Parameter(Mandatory = $true)][string]$Name)
    return "ARYASecurityAuditRole-$Name"
}

function Get-PerUserCustomPolicyName {
    param([Parameter(Mandatory = $true)][string]$Name)
    return "SecurityAuditCustomEdge-$Name"
}
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            "N" {
                ...
                $script:ROLE_NAME = Get-PerUserRoleName -Name $script:ResolvedUserName
                $script:CUSTOM_POLICY_NAME = Get-PerUserCustomPolicyName -Name $script:ResolvedUserName
                ...
                Write-Host "  Using IAM role name      : $($script:ROLE_NAME)" -ForegroundColor Yellow
                continue
            }
            "A" {
...
function Confirm-RoleConflictOrResolve {
    while ($true) {
        ...
        Write-Host ""
        Write-Host "Choose how to continue:" -ForegroundColor Yellow
        Write-Host "  [U] Use the existing IAM role as-is"
        Write-Host "  [P] Patch / update the existing IAM role in-place later in the script"
        ...
        switch ($choice) {
            "U" {
                ...
                $script:RoleCreated = $false
                $script:UseExistingRole = $true
                $script:PatchExistingRole = $false
                Write-Host "  Continuing with existing IAM role without changes: $($script:ROLE_NAME)" -ForegroundColor Yellow
                break
            }
            "P" {
                $script:RoleArn = $existingRole.Role.Arn
                $script:RoleCreated = $false
                $script:UseExistingRole = $true
                $script:PatchExistingRole = $true
                Write-Host "  Continuing with existing IAM role and will patch trust/session settings later: $($script:ROLE_NAME)" -ForegroundColor Yellow
                break
            }
            "R" {
                Remove-IamRoleCompletely -RoleNameToRemove $script:ROLE_NAME -CustomPolicyNameToRemove $script:CUSTOM_POLICY_NAME
                $script:UseExistingRole = $false
                $script:PatchExistingRole = $false
                continue
            }
            "A" {
...
try {
    ...
    $existingCustomPolicyList = Invoke-AwsCli iam list-policies --scope Local --output json
    $existingCustomPolicy = $null
    if ($existingCustomPolicyList.ExitCode -eq 0) {
        $existingCustomPolicy = @((ConvertFrom-AwsJsonOrNull $existingCustomPolicyList.Raw).Policies | Where-Object { $_.PolicyName -eq $script:CUSTOM_POLICY_NAME }) | Select-Object -First 1
    }

    if ($existingCustomPolicy -and $existingCustomPolicy.Arn) {
        $script:CustomPolicyArn = $existingCustomPolicy.Arn
        Write-Host "  ✓ Custom policy already exists: $($script:CustomPolicyArn)" -ForegroundColor Green
    }
    elseif ($PSCmdlet.ShouldProcess("Custom policy '$($script:CUSTOM_POLICY_NAME)'", "Create")) {
    ...
    if ($script:CustomPolicyArn) {
        $customAlreadyAttached = $false
        foreach ($p in $attachedRolePolicies) {
            if ($p.PolicyArn -eq $script:CustomPolicyArn) { $customAlreadyAttached = $true; break }
        }

        if ($customAlreadyAttached) {
            Write-Host "  ✓ Custom policy already attached to role" -ForegroundColor Green
        }
        else {
